...
def debug_cmd():
    ...
    write(
        f'{HexFg("#d7204c")}"{configData.java.java_binary}" {HexFg("#ee38a6")}{jvm_memory[0]} {HexFg("#ee384c")}{jvm_memory[1]}'
    )
    write()
    write(f"{HexFg("#64bea0")}{configData.get_jvm_flags()}")
    write()
    write(
        f'{HexFg("#82bab4")}{configData.get_jar_flags()} {fg.grey}-jar {HexFg("#2a70e8")}"{configData.get_exec_path()}" {HexFg("#72b4d2")}{configData.get_jar_args()}'
    )
    write()
